...
import { ChevronDown, ChevronUp, Calendar, Hotel, Map, Star, Clock, Users, Award, CheckCircle, XCircle, Ticket } from "lucide-react";
...
function PackageDetailsPage() {
  ...
  const getDiscountAwards = () => {
    return userAwards.filter(award =>
      award.awardWon.includes('% discount') && !award.isUsed
    );
  };
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            {pkg.images.tourist.map((img, i) => (
              <div key={i} className="group relative overflow-hidden rounded-xl shadow-md bg-white">
                <img
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Complete travel package</p>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-[#3ab19d] mb-1">
                    {pkg.price.currency} {pkg.price.discountedPrice}
                  </div>
                  {pkg.price.originalPrice && pkg.price.originalPrice !== pkg.price.discountedPrice && (
                ...
                <div className="mt-3 pt-3 border-t border-gray-100">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-bold text-green-600">
                      {pkg.price.currency} {pkg.price.originalPrice - pkg.price.discountedPrice}
                    </span>
                  </div>
                </div>
              )}
          ...
          <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p className="text-sm font-semibold text-[#3ab19d]">{pkg.duration}</p>
              </div>

              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <div className="flex items-center gap-2 mb-1">
                  <Calendar className="w-4 h-4 text-[#3ab19d]" />
                  <span className="text-sm font-medium text-gray-700">Booking Period</span>
                </div>
                <p className="text-xs text-gray-600">{pkg.availability.startDate} - {pkg.availability.endDate}</p>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p className="text-sm font-semibold text-[#3ab19d]">Up to {pkg.availability.maxBookings}</p>
              </div>
            </div>
          </section>
                ...
                <div key={idx} className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="font-medium text-sm text-gray-900">{spot.spotname}</span>
                        <p className="text-xs text-gray-500">{spot.location}</p>
                      </div>
                    </div>
                    {openTourist === idx ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                  </button>
                  {openTourist === idx && (
                ...
                <div key={idx} className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-sm text-gray-900">Day {idx + 1}: {item.title}</span>
                    </div>
                    {openItinerary === idx ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                  </button>
                  {openItinerary === idx && (
                    <div className="px-4 pb-4 border-t border-gray-100">
                      <p className="pt-3 text-gray-600 leading-relaxed text-sm">
                ...
                <div key={idx} className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {openHotel === idx ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                    </div>
                  </button>
                      ...
                      <div className="pt-3 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-900">{hotel.rating}/5 stars</p>
                          </div>
                        </div>
                        {hotel.hotelImages && hotel.hotelImages.length > 0 && (
                         <div className="mt-4">
                         <span className="text-xs font-medium text-gray-700 mb-3 block">Hotel Gallery</span>
              ...
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium text-gray-900">
                        {pkg.price.currency} {pkg.price.discountedPrice || pkg.price.originalPrice}
                      </span>
                    </div>
                    {selectedCoupon && (
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-green-600">Discount Applied ({selectedCoupon.awardWon})</span>
                        <span className="text-sm font-medium text-green-600">
                          -{pkg.price.currency} {(pkg.price.discountedPrice || pkg.price.originalPrice) - calculateFinalPrice()}
                        </span>
                      </div>
                    )}
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    {selectedCoupon ? `Applied: ${selectedCoupon.awardWon}` : 'Use Discount Coupon'}
                  </button>
                  {selectedCoupon && (
                    <button
                      type="button"
              ...
              <div className="space-y-4">
                <div className="bg-[#dda169]/5 p-3 rounded-lg border border-[#dda169]/20">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-bold text-[#dda169]">{pkg.price.currency} {pkg.price.originalPrice}</span>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {getDiscountAwards().length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-500">No discount coupons available</p>
                        <p className="text-sm text-gray-400 mt-1">Win challenges to earn discount coupons!</p>
                      </div>
                    ) : (
                      getDiscountAwards().map((award) => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              <h3 className="font-semibold text-gray-900">{award.awardWon}</h3>
                              <p className="text-sm text-gray-600">{award.challengeTitle}</p>
                              <p className="text-xs text-gray-500 mt-1">{award.awardDescription}</p>
                            </div>
                            <div className="text-right">
                              ...
                              <div className="text-xs text-gray-500">
                                Won {new Date(award.awardedAt).toLocaleDateString()}
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              <div className="space-y-2">
                {pkg.inclusions?.length > 0 ? (
                  pkg.inclusions.map((item, idx) => (
                    <div key={idx} className="flex items-start gap-3 p-2 bg-green-50 rounded-lg border border-green-100">
                  ...
                  <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <span className="text-sm text-gray-500">Details not provided</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border-2 border-dashed border-yellow-300 rounded-xl p-4 mb-6">
                <div className="text-center">
                  ...
                  <h4 className="text-sm font-semibold text-gray-900 mb-1">Your Booking Coupon</h4>
                  ...
                  <p className="text-xs text-gray-600">Keep this coupon number safe for your records</p>
                </div>
              </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  ...
                  <span className="text-sm font-semibold text-gray-900">#{Math.floor(Math.random() * 1000000)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-100">
                  <span className="text-sm text-gray-600">Travelers</span>
                  <span className="text-sm font-semibold text-gray-900">2 Adults</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-gray-600">Total Paid</span>
                  <span className="text-lg font-bold text-emerald-600">{pkg.price.currency} {(pkg.price.discountedPrice || pkg.price.originalPrice) * 2}</span>
                </div>
              </div>
